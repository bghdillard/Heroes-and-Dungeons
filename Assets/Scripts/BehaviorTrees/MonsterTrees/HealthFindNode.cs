...
public class HealthFindNode : Node
{
    Monster monster;

    public HealthFindNode(Monster monster) : base()
    {
    ...
    {
        if (parent.parent.GetData("StaminaRestore") != null || parent.parent.GetData("MagicRestore") != null) return NodeState.FAILURE;
        if (parent.GetData("Target") != null) return NodeState.SUCCESS;
        if (!monster.GetHealthStatus()) return NodeState.FAILURE;
        List<Restorative> restoratives = GridManager.GetRestoratives("Health");
